...
class SegFormerEncoder(nn.Module):
    def __init__(
        ...
        self.base.append(
            ConvLayer(in_channels=3, out_channels=first_ch[0], kernel=3, stride=1, dilation_this=1))  # conv0
...
class SegHarDNet(nn.Module):
    def __init__(self, n_classes=19, n_channels_reg=3):
        ...
        self.conv1x1_up.append(ConvLayer(cur_channels_count, cur_channels_count // 2,
                                         kernel=1))  ### HarD Block output added to skip connections
    ...
    def forward(self, x):
        ...
        out_seg_final = F.interpolate(out_seg, size=(540,960), mode="bilinear", align_corners=True)

        out_centerline_final = F.interpolate(out_centerline, size=(540,960), mode="bilinear",
                                             align_corners=True)

        out_AFM_final = F.interpolate(out_AFM, size=(540,960), mode="bilinear", align_corners=True)

        if self.n_channels_reg == 3:
            out_leftright_final = F.interpolate(out_leftright, size=(size_in[2], size_in[3]), mode="bilinear",
                                                align_corners=True)
